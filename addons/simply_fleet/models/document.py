...
class Document(models.Model):
   ...
   # Changed from Char to Many2one field to select from predefined document names
   document_name_id = fields.Many2one(
       'simply.fleet.document.name', 
       string='Document Name', 
       required=True, 
       tracking=True
   )
   
   # Keep the old name field for backward compatibility but make it computed
   name = fields.Char(
       string='Document Name (Text)', 
       related='document_name_id.name', 
       store=True, 
       readonly=True
   )
   ...
   # Changed from Char to Many2one field to select from predefined authorities
   issuing_authority_id = fields.Many2one(
       'simply.fleet.issuing.authority', 
       string='Issuing Authority', 
       tracking=True
   )
   
   # Keep the old issuing_authority field for backward compatibility but make it computed
   issuing_authority = fields.Char(
       string='Issuing Authority (Text)', 
       related='issuing_authority_id.name', 
       store=True, 
       readonly=True
   )
